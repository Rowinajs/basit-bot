...
let rol = member.guild.roles.cache.find(r => r.name === "Verifed");
...
member.send("> Onaylıken Sunucudan Çıktığın için Yeniden Onaylı Rolü Verildi!")
 setTimeout(function(){
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        const kufur = ["dininizi", "dinini", "inanc", "inanç", "ırkını", "hileci", "oç", "amk", "ananı sikiyim", "ananıskm", "hile", "amk", "amsk", "sikim", "sikiyim", "hileci", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "ak", "amq","sg",];
        if (kufur.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                          
                      return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Lütfen Discord Kurallarına Dikkat Ediniz.`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
          ...
          try {
            if (!oldMessage.member.hasPermission("BAN_MEMBERS")) {
                  oldMessage.delete();
                      ...
                      return oldMessage.channel.send(new Discord.MessageEmbed().setDescription(`${oldMessage.author} Lütfen Discord Kurallarına Dikkat Ediniz.`).setColor('0x800d0d').setAuthor(oldMessage.member.displayName, oldMessage.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
